		...
		for (Object sLobj : serviceList) {
			...
			try {
				JsonUtil gDL = new JsonUtil((JSONObject) CommonUtil.getData(_serviceList,
						_serviceList.getString("getDeviceList_url_addr") + "&deviceType=S"));
							...
							if ((MVLList.length() > 0)) {
								for (Object MVLobj : MVLList) {
									...
									if (MVLitem.has("rainType")) {
										Find_wMap(tMap, "rainType").put("value", MVLitem.optString("obsrValue",
												WeatherType.findBy(MVLitem.optInt("obsrValue", 0)).getEngNm()));
									} else {
									...
									if (MVLitem.has("hourlyRainfall")) {
										Find_wMap(tMap, "hourlyRainfall").put("value",
												MVLitem.optDouble("hourlyRainfall"));
									} else {
