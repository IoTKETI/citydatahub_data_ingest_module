...
@Slf4j
public class ConvAirQualityMeasurement_Static_SiheungLivingLab extends AbstractConvert {
						...
						if (gMVL.has("list")) {
							...
							for (Object MVLobj : MVLList) {
								...
								if (MVLitem.has("pm10Value") || MVLitem.has("pm10"))
									Find_wMap(tMap, "pm10").put("value",
											MVLitem.optDouble("pm10Value", MVLitem.optDouble("pm10", 0.0d)));
								else
									Delete_wMap(tMap, "pm10");

								if (MVLitem.has("pm25Value") || MVLitem.has("pm25"))
									Find_wMap(tMap, "pm25").put("value",
											MVLitem.optDouble("pm25Value", MVLitem.optDouble("pm25", 0.0d)));
								else
								...
								if (MVLitem.has("pm10Grade1h"))
									Find_wMap(tMap, "pm10GradeHour1").put("value",
											Grade(MVLitem.optInt("pm10Grade1h", 0)));
								else
									Delete_wMap(tMap, "pm10GradeHour1");

								if (MVLitem.has("pm25Grade1h"))
									Find_wMap(tMap, "pm25GradeHour1").put("value",
											Grade(MVLitem.optInt("pm25Grade1h", 0)));
								else
