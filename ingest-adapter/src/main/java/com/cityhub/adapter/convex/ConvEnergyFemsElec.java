...
public class ConvEnergyFemsElec extends AbstractConvert {
	...
	public String doit() throws CoreException {
		...
		try {
			...
			for (int i = 0; i < svcList.length(); i++) {
				...
				if(ju.has("header") && "SUCCESS".equals(ju.get("header.result")) &&
					ju.has("body.items") && ju.getArray("body.items").length() > 0) {
	...
	private JSONObject getData(JSONObject svc) throws Exception {
		...
		OkHttpClient client = new OkHttpClient.Builder().connectTimeout(3, TimeUnit.SECONDS)
				.writeTimeout(3 * 10, TimeUnit.SECONDS).readTimeout(3 * 10, TimeUnit.SECONDS).build();
