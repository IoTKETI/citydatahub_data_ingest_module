...
@Slf4j
public class ConvWeatherMeasurement_Aws_Siheung extends AbstractConvert {
									...
									for (Object obj : arrList) {
										...
										if (item != null) {							
											...
											Map<String, Object> tMap = objectMapper
													.readValue(
															templateItem.getJSONObject(ConfItem.getString("modelId"))
																	.toString(),
											...
											if (item.has("LOCTN_ATM_INFO")
													&& (item.optDouble("LOCTN_ATM_INFO") > -99.0d)) {
												Find_wMap(tMap, "atmosphericPressure").put("value",
														item.optDouble("LOCTN_ATM_INFO"));
											} else {
												Delete_wMap(tMap, "atmosphericPressure");
											}
											if (item.has("WTRLVL_ATM_INFO")
													&& (item.optDouble("WTRLVL_ATM_INFO") > -99.0d)) {
												Find_wMap(tMap, "seaLevelPressure").put("value",
														item.optDouble("WTRLVL_ATM_INFO"));
											} else {
											...
											if (item.has("RAINF_YN_INFO")
													&& (Double.parseDouble(item.optString("RAINF_YN_INFO")) > -99.0d)) {
												Find_wMap(tMap, "rainfall").put("value",
														item.optString("RAINF_YN_INFO"));
											} else {
												Delete_wMap(tMap, "rainfall");
											}

											if (item.has("RAINF_1HR_INFO")
													&& (item.optDouble("RAINF_1HR_INFO") > -99.0d)) {
												Find_wMap(tMap, "hourlyRainfall").put("value",
														item.optDouble("RAINF_1HR_INFO"));
											} else {
												Delete_wMap(tMap, "hourlyRainfall");
											}
											if (item.has("RAINF_DAY_INFO")
													&& (item.optDouble("RAINF_DAY_INFO") > -99.0d)) {
												Find_wMap(tMap, "dailyRainfall").put("value",
														item.optDouble("RAINF_DAY_INFO"));
											} else {
											...
											if (item.has("insolation")) {
												Find_wMap(tMap, "insolation").put("value",
														item.optDouble("insolation"));
											} else {
											...
											if (item.has("snowfallHour3")) {
												Find_wMap(tMap, "snowfallHour3").put("value",
														item.optDouble("snowfallHour3"));
											} else {
												Delete_wMap(tMap, "snowfallHour3");
											}
											if (item.has("visibility")) {
												Find_wMap(tMap, "visibility").put("value",
														item.optDouble("visibility"));
											} else {
											...
											wMap.put("value",
													iSvc.optString("dataProvider", "https://www.weather.go.kr"));
											...
											Map<String, Object> addrValue = (Map) ((Map) tMap.get("address"))
													.get("value");
