							...
							if (item != null) {
								...
								if (item.has("pm10Value") || item.has("pm10"))
									Find_wMap(tMap, "pm10").put("value",
											item.optDouble("pm10Value", item.optDouble("pm10", 0.0d)));
								else
									Delete_wMap(tMap, "pm10");

								if (item.has("pm25Value") || item.has("pm25"))
									Find_wMap(tMap, "pm25").put("value",
											item.optDouble("pm25Value", item.optDouble("pm25", 0.0d)));
								else
