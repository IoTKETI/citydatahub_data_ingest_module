...
public class ConvAirQualityMeasurementStationary extends AbstractConvert {
  ...
  public String doit() throws CoreException {
    ...
    try {
      ...
      for (int i = 0; i < svcList.length(); i++) {
        ...
        if ("00".equals(jsonData.getString("resultCode")) && "SUCCESS".equals(jsonData.getString("resultMsg")) && ju.has("list")) {
  ...
  /*
   * private String urlAssemble(JSONObject svc) { String url =
   * svc.getString("url_addr"); String[] paramVariable =
   * svc.getString("ParamVariable").split(","); String value = "";
   *
   * if (svc.has("key_name") && svc.has("key_value")) { url += "&" +
   * svc.getString("key_name") + "=" + svc.getString("key_value"); }
   *
   * for (String it : paramVariable) { if (!"".equals(it) && svc.has(it)) { value
   * = svc.getString(it) + ""; if (value.indexOf(",") > -1) { String[] val =
   * value.split(","); if (DateUtil.isPatternDate(val[0])) { url += "&" + it + "="
   * + DateUtil.addDate(DateUtil.getChronoUnit(val[1]), Integer.parseInt(val[2]),
   * val[0]); } } else { if (DateUtil.isPatternDate(value)) { url += "&" + it +
   * "=" + DateUtil.getDate(value); } else { url += "&" + it + "=" + value; } } }
   * else { if (svc.has("key")) { url += "&" + it + "=" + svc.getString("key"); }
   * } } return url; }
   *
   * private Headers buildHeader(JSONObject svc) { Map h = new HashMap();
   * JSONArray headers = svc.getJSONArray("headers");
   *
   * for (Object obj : headers) { JSONObject jobj = (JSONObject) obj;
   * h.putAll(jobj.toMap()); }
   *
   * return Headers.of(h); }
   */
