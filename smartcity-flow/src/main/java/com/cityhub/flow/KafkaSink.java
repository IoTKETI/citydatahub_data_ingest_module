...
/**
 * A Flume Sink that can publish messages to Kafka. This is a general implementation that can be used with any Flume agent and a channel. The message can be any event and the key
 * is a string that we read from the header For use of partitioning, use an interceptor to generate a header with the partition key
 * <p/>
 ...
 * <p/>
 * Optional properties topic - there's a default, and also - this can be in the event header if you need to support events with different topics batchSize - how many messages to
 * process in one batch. Larger batches improve throughput while adding latency. requiredAcks -- 0 (unsafe), 1 (accepted by at least one broker, default), -1 (accepted by all
 * brokers) useFlumeEventFormat - preserves event headers when serializing onto Kafka
 * <p/>
...
public class KafkaSink extends AbstractSink implements Configurable, BatchSizeSupported {
   ...
   *
   * Kafka producer properties is generated as follows: 1. We generate a properties object with some static defaults that can be overridden by Sink configuration 2. We add the
   * configuration users added for Kafka (parameters starting with .kafka. and must be valid Kafka Producer properties 3. We add the sink's documented parameters which can override
   * other properties
   *
   * @param context
   */
